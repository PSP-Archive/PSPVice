...
int autostart_prg(const char *file_name, unsigned int runmode)
{
    ...
    #ifdef PSP
    PSPStoreP00Filename(file); // Directory and file
    #endif
    ...
    reboot_for_autostart((char *)(finfo->name), AUTOSTART_HASDISK, runmode); // P00 : This is P00 header name
    //printf("reboot_for_autostart %s\n", finfo->name); // CK debug
