...
#else
#include "string.h"
  // PSP, No dir mangament
int ioutil_access(const char *pathname, int mode) { return 0; };
int ioutil_chdir(const char *path) { return 0;};
char *ioutil_getcwd(char *buf, int size) { return "";};
int ioutil_isatty(int desc) { return 0; };
#ifndef PATH_MAX
#  ifdef MAX_PATH
#    define PATH_MAX MAX_PATH
#  else
...
int ioutil_mkdir(const char *pathname, int mode) { return archdep_mkdir(pathname, mode); };
int ioutil_remove(const char *name) { return 1; };
int ioutil_rename(const char *oldpath, const char *newpath) { return 1; };
int ioutil_stat(const char *file_name, unsigned int *len,unsigned int *isdir) { return archdep_stat(file_name, len, isdir); };
struct ioutil_dir_s;
struct ioutil_dir_s *ioutil_opendir(const char *path) {return NULL;};
char *ioutil_readdir(struct ioutil_dir_s *ioutil_dir) { return NULL;};
void ioutil_closedir(struct ioutil_dir_s *ioutil_dir) {};
char *ioutil_current_dir(void)
{
    static size_t len = 128;
    char *p = (char *)lib_malloc(len);
