...
static char *p00_file_find(const char *file_name, const char *path)
{
  ...
  char* name;
  char* newname;
  name = PSPGetP00Filename();
  newname = lib_stralloc(name); // This function return an allocated string.
  return newname;
  #endif
                        ...
                        unsigned int command, unsigned int open_type)
{
    ...
    printf("p00_open"); // CK Debug

    if (command & FILEIO_COMMAND_FSNAME)
    {
        printf("p00_open Mode FS (use filename as it is)\n");
        fname = lib_stralloc(file_name);
    } else {
        switch (command & FILEIO_COMMAND_MASK) {
          printf("p00_open Mode MASK (find P00 name from header name)\n");
          case FILEIO_COMMAND_READ:
          case FILEIO_COMMAND_APPEND:
          case FILEIO_COMMAND_APPEND_READ:
            // TODO This fail on PSP !!!!
            fname = p00_file_find(file_name, path); // CK : Here the real file name "xx.p00" is find from header name (all directory files are browsed)
            break;
    ...
    if (fname == NULL)
    {
        printf("Error, P00 filename not from from header name"); // CK
        return NULL;
    }

    printf("p00_open fname %s", fname); // CK Debug // fname is filename "xx.p00".
    ...
    if (rawfile == NULL)
    {
        printf("Error, can not open raw file %s, path %s\n", fname, path);
...
*/
    ...
    info->name = (BYTE *)lib_stralloc(rname); // rname = header name in p00 file (different from file name)
    ...
    //printf("rname %s, lenght %d, type %d, format %d", info->name, info->length, info->type, info->format); // CK Debug
