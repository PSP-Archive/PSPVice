                              ...
                              cbmdos_cmd_parse_t *cmd_parse, char *rname)
{
    ...
    printf("fsdevice_open_file"); // CK Debug
    ...
    printf("Prepare for buffered reads"); // CK
    
    if (tape_image_open(tape) < 0) {
        lib_free(tape->name);
        tape->name = NULL;
    } else {
        //printf("Else et sortie, pas bon"); // CK
    ...
    printf("Call fileio_open"); // CK

    // Ici on a rname = header name, format = 3
    finfo = fileio_open(rname, fsdevice_get_path(vdrive->unit), format,
